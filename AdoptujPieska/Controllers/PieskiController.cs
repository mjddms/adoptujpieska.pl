...
>>>>>>> 9a70e50cb92ab132ea9b9eb0f5a289b2cb4014b6
        ...
        [HttpPost]
        public ActionResult CreateForm(int id, string name, string surname, string phone, string message)
        {
            ...
            db.SubmitChanges();
            return RedirectToAction("Profile", "Pieski", new { id = id });

        }

        public ActionResult DeleteForm(int id)
        {
            var FormToDelete = db.FormSchel.FirstOrDefault(p => p.Id == id);
            if (FormToDelete != null)
            {
                db.FormSchel.DeleteOnSubmit(FormToDelete);
                db.SubmitChanges();
            }
        ...
        public ActionResult GetDogImageById(int id)
        {
            Pieski piesek = db.Pieski.SingleOrDefault(x => x.Id == id);
            if (piesek == null)
            {
            ...
            return File(piesek.Zdjecie, "image/jpeg"); // Zwraca plik obrazu (typ MIME: image/jpeg)
        }
